...
class Snapshot : public QObject
{
     ...
     *          "last_changed_date" --- дата последнего изменения директории/файла/подпапки
     *          "inner_files" --- список снапшотов внутренних файлов директории/подпапки
     *          "alt" --- список альтернативных потоков с информацией о них.
     *                     Ключи для списка alt:
     *                     "alt_name" --- имя альтернативного потока
     *                     "alt_size" --- длина потока в байтах
     *                     "alt_hash_sum" --- контрольная сумма потока
     */
     ...
     *  пара элементов (контрольная сумма файла, словарь с инфорацией о альтпотоках (ключ - имя потока, значение- контрольная сумма)).
     * @details Рекурсивно проходит по массиву внутренних файлов снапшота, записывая в словарь путь и контрольную сумму и словарь
     *          с информацией об альтернативных потоках
   ...
   // QMap<QString, QString> createCompareMap (QJsonObject obj, QString& parent_dir, QMap<QString, QString>& result);
    QMap<QString, QPair<QString, QMap<QString, QString>>> createCompareMap (QJsonObject obj, QString& parent_dir,
                                                                                      QMap<QString, QPair<QString, QMap<QString, QString>>>& result);
    /**
     * @brief Функция, собирающая вместе информацию об альтенативных потоков файла/папки.
     * @param filePath --- путь до файла/папки для которого мы ищем альт.потоки.
     * @param hashAlgorithm --- алгоритм хэширования, использующийся для составления контрольной суммы.
     * @return Список QJsonArray, состоящий из элементов QJsonobject, сохраняющий информацию об альтюпотоках
     *          имеет следующие поля:
     *                               "alt_name" --- имя альтернативного потока
     *                               "alt_size" --- длина потока в байтах
     *                               "alt_hash_sum" --- контрольная сумма потока
     */
    QJsonArray checkForAltDS(QString filePath, ALG_ID hashAlgorithm);
...
public:
     ...
     *          Если состояние файла в двух снапшотах различны, информация об этом (структура: путь до файла, флаг состояния,
     *          является ли альт.потоком, имя альт.потока)
